...
public class TestOracleAndImpalaFile {
    ...
    public static void main(String[] args) throws Exception {
		  ...
		  String ddlSnapshotTableDir =
		  URLDecoder.decode(TestOracleAndImpalaFile.class.getClassLoader().getResource(
		  "").getPath(), "utf-8") + "/020301 Oracle_当日_建表"; String ddlPartitionTableDir
		  =
		  URLDecoder.decode(TestOracleAndImpalaFile.class.getClassLoader().getResource(
		  "").getPath(), "utf-8") + "/020302 Oracle_历史_建表"; String
		  dmlSnapShotToPartitionDir =
		  URLDecoder.decode(TestOracleAndImpalaFile.class.getClassLoader().getResource(
		  "").getPath(), "utf-8") + "/kettle_project/oracle"; FileUtils.forceMkdir(new
		  File(ddlSnapshotTableDir)); FileUtils.forceMkdir(new
		  File(ddlPartitionTableDir)); FileUtils.forceMkdir(new
		  File(dmlSnapShotToPartitionDir));
		  
		  SrcdbToTgtdb srcdbToTgtdb = new SrcdbToTgtdb(ddlSnapshotTableDir,
		  ddlPartitionTableDir, dmlSnapShotToPartitionDir);
